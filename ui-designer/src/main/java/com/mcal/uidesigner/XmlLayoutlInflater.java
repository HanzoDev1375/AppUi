...
import com.mcal.uidesigner.helper.BinderView;
...
public abstract class XmlLayoutlInflater implements UndoManager.UndoRedoListener {
  ...
  private void configureView(View view, boolean editMode) {
    ...
    if (view.getClass() == RecyclerView.class) {
      setAd((RecyclerView) view);
    }
  }

  void setAd(RecyclerView view) {

    view.setLayoutManager(new LinearLayoutManager(context));
    view.setAdapter(new BinderView());
  ...
  private View inflateView(Node node, @NonNull String elementName) {
    ...
    if (elementName.contains("androidx.recyclerview.widget.RecyclerView")) {

      try {
        return new RecyclerView(context, (AttributeSet) null, (Integer) 0);
      } catch (Throwable e) {
        e.printStackTrace();
        return null;
      }
  ...
  private void inflateAttributes(
      PropertyObject obj, Node node, @NonNull List<XmlLayoutProperties.PropertySpec> properties) {
    int min;
  ...
  private String getMenuReferenceAttributeValue(
      Node node, XmlLayoutProperties.PropertySpec property) {
  ...
  public void pastLayout() {

    layoutPast.onPastLayout(getXml());
  }
}
